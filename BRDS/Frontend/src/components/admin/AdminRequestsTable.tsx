...
export function AdminRequestsTable() {
  ...
  const getStatusPill = (status: string) => {
    ...
    if (s === "pending") return <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-700">Pending</span>;
    if (s === "under review" || s === "review") return <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">Under Review</span>;
    if (s === "approved") return <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-700">Approved</span>;
    if (s === "released") return <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-700">Released</span>;
    if (s === "needs info") return <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-orange-100 text-orange-700">Needs Info</span>;
    if (s === "rejected") return <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700">Rejected</span>;
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50/50">
              ...
              <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Resident Name</th>
      ...
      <div className="p-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500 bg-white rounded-b-xl">
        <p>Showing 1 to {requests.length} of {requests.length} results</p>
        <div className="flex items-center gap-1">
          <button className="px-3 py-1.5 border border-gray-200 rounded text-gray-600 hover:bg-gray-50" disabled>Previous</button>
          <button className="px-3 py-1.5 border border-emerald-500 bg-emerald-50 text-emerald-700 rounded">1</button>
