...
export function DashboardPage() {
  ...
  const calculateStats = (reqs: any[]) => {
    let active = 0, ready = 0, completed = 0;
    reqs.forEach(r => {
    ...
    if (s === 'ready for pickup') {
      ...
      return (
        <div className="px-3 py-1.5 rounded-full border border-gray-200 bg-white text-gray-700 text-xs font-semibold flex items-center gap-1.5 capitalize">
          <Clock className="w-3.5 h-3.5 text-gray-400" /> {status || 'Submitted'}
        </div>
      );
    }
  };
      ...
      <div className="max-w-4xl w-full space-y-6">
        ...
        <div className="bg-emerald-500 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full border-2 border-emerald-400 flex items-center justify-center bg-emerald-600/20 shrink-0">
              <User className="w-7 h-7 text-white" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-bold text-white">Kumusta, {firstName}!</h1>
                <button className="w-6 h-6 rounded-full bg-emerald-400/40 flex items-center justify-center hover:bg-emerald-400/60 transition-colors">
                  <Pencil className="w-3 h-3 text-white" />
                </button>
              </div>
              <p className="text-emerald-100 text-sm mt-1">
                {profile?.address || "Address not set"} | {profile?.phone_number || ""}
              </p>
            </div>
          </div>
          <Button 
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex items-center justify-between">
            <h2 className="text-lg font-bold text-gray-900">Mga Nakaraang Request</h2>
          </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-bold text-gray-900 mb-1 capitalize">{req.document_type?.replace(/-/g, ' ') || 'Document'}</h3>
